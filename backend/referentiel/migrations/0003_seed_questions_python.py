...
# Données figées (AUDIT-011) : ne pas remplacer par un import applicatif.
...
def charger_referentiel(apps, schema_editor):
    Referentiel = apps.get_model('referentiel', 'Referentiel')
    Theme = apps.get_model('referentiel', 'Theme')
    Question = apps.get_model('referentiel', 'Question')
    ChoixReponse = apps.get_model('referentiel', 'ChoixReponse')

    referentiel, _ = Referentiel.objects.update_or_create(
        nom=REFERENTIEL['nom'],
        version=REFERENTIEL['version'],
        defaults={
            'description': REFERENTIEL['description'],
            'score_maximum': REFERENTIEL['score_maximum'],
            'actif': True,
        },
    )
    themes = {}
    for code, nom, description, ordre in THEMES:
        themes[code], _ = Theme.objects.update_or_create(
            referentiel=referentiel,
            code=code,
            defaults={'nom': nom, 'description': description, 'ordre': ordre},
        )

    for code, numero, texte, ref_iso, ref_nist, ref_loi, recommandation, ordre in QUESTIONS:
        ...
        for valeur, libelle, texte_choix in CHOIX_PAR_DEFAUT:
            ChoixReponse.objects.get_or_create(
                question=question,
                valeur=valeur,
                defaults={'libelle_niveau': libelle, 'texte': texte_choix},
            )


def vider_referentiel(apps, schema_editor):
    Referentiel = apps.get_model('referentiel', 'Referentiel')
    Referentiel.objects.filter(
        nom='Référentiel enrichi d\'évaluation de maturité cybersécurité PME',
        version='1.0',
    ).delete()
...
class Migration(migrations.Migration):
    dependencies = [('referentiel', '0002_question_recommandation')]
    operations = [migrations.RunPython(charger_referentiel, vider_referentiel)]
